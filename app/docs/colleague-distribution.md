...
**Try the app:** install Node.js 24, extract the release ZIP and run the root
`Start-Hackathon.cmd` on Windows, or `node app/scripts/start-demo.mjs`.
The synthetic local demo requires no Copilot subscription or Azure credentials.

Created by Daniel Horvath · dahorvath@microsoft.com.

**No technical setup needed:** use the Word playbook, PowerPoint workshop or PDFs
at https://github.com/HorvathDaniel-2002/hackathon-facilitator-agent/tree/main/materials .
The templates and checklists work manually without the agent or Copilot.
...
workspace app for customer AI hackathons. The agent offers general sponsorship
and agenda planning; the app provides use-case intake, evidence-based scoping,
technical readiness, Kanban tracking, demos and ownership handoffs. Charter and
standalone Milestone storage are no longer app capabilities. It distinguishes project defaults from verified Microsoft
