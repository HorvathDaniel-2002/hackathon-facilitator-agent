...
brings these activities into a reusable workflow: aligned outcomes, structured use cases,
...
- Workspace-scoped collaboration roles, version-checked edits, Kanban tracking and
  preserved historical artifacts.
- Readout/handoff exports and follow-up/cleanup ownership.

## What is implemented

The local application supports workspace settings, intake, contacts and teams, evaluation
workflow, saved guide generation, runbook checks, Kanban progress, handoff and
...
The public repository distributes the sanitized Kanban-first app, local-demo
launcher, optional Copilot agent, reference pack and no-install materials:
https://github.com/HorvathDaniel-2002/hackathon-facilitator-agent .
The app is under `app/`; databases and customer records are excluded.
Reviewers can download without an invitation. Node.js 24 is required for the
...
review on 2026-09-11 recorded **409 unit/API tests and 53 browser tests passing**, plus
TypeScript, lint and production packaging. Those historical counts predate the
Kanban and feature-removal changes; re-run current validation before demonstrating
the updated app. Test counts are software regression evidence, not model accuracy,
